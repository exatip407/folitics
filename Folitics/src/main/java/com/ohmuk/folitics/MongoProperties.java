...
public class MongoProperties {
     ...
     */
    private String uri = "mongodb://localhost:27017/folitics";
    ...
    /**
     * Creates a {@link MongoClient} using the given {@code options} and {@code environment}. If the configured port is
     * zero, the value of the {@code local.mongo.port} property retrieved from the {@code environment} is used to
        ...
        try {
            if (hasCustomAddress() || hasCustomCredentials()) {
                ...
                if (hasCustomCredentials()) {
                    ...
                    credentials.add(MongoCredential.createMongoCRCredential(this.username, database, this.password));
                }
    ...
    private int determinePort(Environment environment) {
        ...
        if (this.port == 0) {
            ...
            throw new IllegalStateException("spring.data.mongodb.port=0 and no local mongo port configuration "
                    + "is available");
        }
        return this.port;
    }
